...
(function(global, $){
    ...
    codiad.filemanager = {
        ...
        saveModifications: function(path, data, callbacks){
            ...
            // --- Save webhook
            var webhookController = 'http://docker/api/webhook/codiad/';
            var webhookController = 'http://localhost:8000/api/webhook/codiad/';
            var data = {
                path: path,
                theme_or_project: 'themes'
            };

            $.post(webhookController + 'savedFile', data, function(resp){
                console.log(resp);
            });
