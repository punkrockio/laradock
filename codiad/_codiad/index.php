...
<body>
    ...
    <script type="text/javascript">
        var app = $.sammy(function() {
          ...
          this.get('/#/:username/:projectname/:filename', function(context) {
                var username    = this.params['username'],          
                    projectname = this.params['projectname'],          
                    filename    = this.params['filename'];          
                var path        = username + '/' + projectname + '/' + filename;

                // --- Open file
                codiad.filemanager.openFile(path);

                // --- Open sidenbav
                setTimeout(function() {

                    var sidebar         = $('[data-path="'+username+'/'+projectname+'"]');
                    var sidebarParent   = sidebar.closest('li');
                    var sidebarIcon     = sidebarParent.find('span');

                    sidebarIcon.click();
                    $('.activator').removeClass('activator');
                    sidebarParent.addClass('activator');


                    $('#file-manager').animate({
                        scrollTop: sidebarParent.offset().top - 35
                    }, 2000);
    ...
    <?php
        ...
        echo('<script> $(document).ready(function() { app.run(); }) </script> ');
